...
export interface IClient {
    ...
    getApiUsersUserRoles(): Promise<RolesEnum[]>;
...
export class Client implements IClient {
    ...
    getApiUsersUserRoles(signal?: AbortSignal): Promise<RolesEnum[]> {
        let url_ = this.baseUrl + "/api/Users/userRoles";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetApiUsersUserRoles(_response);
        });
    }

    protected processGetApiUsersUserRoles(response: Response): Promise<RolesEnum[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as RolesEnum[];
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<RolesEnum[]>(null as any);
    }
